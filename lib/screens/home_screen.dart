...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.black,
        elevation: 0,
        title: const Text("Minox Plan", style: TextStyle(color: Color(0xFFD4AF37), fontWeight: FontWeight.bold)),
        actions: [
          IconButton(
      ...
      body: StreamBuilder<DocumentSnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          String aiRoutineText = userData['ai_generated_routine'] ?? "Creando tu plan..."; // <--- AQUÍ LEE A LA IA
                    ...
                    children: [
                      const Row(
                        children: [
                          ...
                          SizedBox(width: 10),
                          Text("Tu Estrategia Personal", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                        ],
                      ),
  ...
  Icon _getIconForProduct(String productName) {
    if (productName.contains("Minoxidil")) return const Icon(Icons.science, color: Color(0xFFD4AF37));
    if (productName.contains("Aceite")) return const Icon(Icons.water_drop, color: Colors.amber);
    if (productName.contains("Bálsamo")) return const Icon(Icons.spa, color: Colors.green);
    if (productName.contains("Cera")) return const Icon(Icons.face, color: Colors.orange);
    if (productName.contains("Protector")) return const Icon(Icons.wb_sunny, color: Colors.yellow);
    if (productName.contains("Dermaroller")) return const Icon(Icons.grid_on, color: Colors.grey);
